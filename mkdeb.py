...
def create_control(package, version, arch, description, depends):
    with open(deb_dir + "/" + "control", mode='x') as f:
        ...
        f.write("\n")
        f.writelines("Maintainer: support <support@netskyper.com>")
        f.write("\n")
...
def create_ipkg(name, version, arch):
    cwd = os.getcwd()
    os.chdir(os.path.join(cwd, 'tmp'))

    os.system('cp -r ./DEBIAN ./control')
    os.mkdir('./data')
    os.system('cp -r ./opt ./data')
    os.system('cp -r ./etc ./data')
    os.system('cp -r ./usr ./data')
    os.system('cp -r ./tmp ./data')
    os.system('''echo "2.0" > ../tmp/debian-binary''')
    os.chdir(os.path.join(cwd, 'tmp/control'))
    os.system('tar --owner=root --format=gnu -zcvf ../control.tar.gz .')
    os.chdir(os.path.join(cwd, 'tmp/data'))
    os.system('tar --owner=root --format=gnu -zcvf ../data.tar.gz .')
    os.chdir(os.path.join(cwd, 'tmp'))
    os.system(
        'tar --owner=root --format=gnu -zcvf ../xxx.tar.gz ./control.tar.gz ./data.tar.gz ./debian-binary')
    os.system('mv ../xxx.tar.gz ' + '../' + name+"_"+version+"_"+arch+".ipk")
